...
const STEP_INFO = [
  { title: "权限引擎概览", desc: "每次文件操作都经过 260KB+ 的权限系统检查" },
  { title: "规则定义", desc: "权限规则用 glob 模式匹配路径，支持 allow/deny/ask 三种策略" },
  { title: "检查: 读取 src 文件", desc: "src/auth.ts 匹配 src/** 规则 → 允许读取" },
  { title: "检查: 编辑 src 文件", desc: "同一文件的编辑操作也匹配 allow 规则" },
  { title: "检查: 编辑 node_modules", desc: "node_modules 下的文件禁止编辑 → 拒绝" },
  { title: "检查: 读取 .env", desc: ".env* 模式匹配环境变量文件 → 拒绝（防止泄露密钥）" },
  { title: "检查: 读取用户目录", desc: "~/Documents 匹配 ~/** → 需询问用户" },
  { title: "环境变量清洗", desc: "子进程启动前清除敏感环境变量，防止通过 env 泄露" },
];
...
export default function PermissionsVisualization() {
  ...
  return (
    <section className="min-h-[520px] space-y-4">
      <h2 className="text-xl font-semibold text-zinc-100">权限引擎</h2>
      ...
      <div className="rounded-lg border border-zinc-700 bg-zinc-900 p-4">
        <div className="flex flex-col gap-4 lg:flex-row">
          ...
          <div className="w-full lg:w-[55%]">
            ...
            {viz.currentStep === 7 && (
              <motion.div initial={{opacity:0}} animate={{opacity:1}} className="space-y-1.5">
                <div className="font-mono text-xs text-zinc-500 mb-2">subprocessEnv — 环境变量清洗</div>
                {ENV_VARS.map((v, i) => (
                  <motion.div
                    key={v.name}
          ...
          <div className="w-full lg:w-[45%]">
            <div className="mb-2 font-mono text-xs text-zinc-500">检查记录</div>
